...
export function updateProgressCounter() {
    ...
    let slideNrFormatted = window.growtype_quiz_global.current_question_nr < 10 ? '0' + window.growtype_quiz_global.current_question_nr : window.growtype_quiz_global.current_question_nr;
    ...
    if ($('.growtype-quiz-question-nr').attr('data-counter-style') === 'steps' || $('.growtype-quiz-question-nr').attr('data-counter-style') === 'outof') {
        ...
        slideNrFormatted = window.growtype_quiz_global.current_question_nr;
    }

    if ($('.growtype-quiz-question-nr').attr('data-counter-style') === 'answered_only') {
        slideTotalNrFormatted = window.quizQuestionsAmount;
        slideNrFormatted = window.growtype_quiz_global.current_question_nr - 1;
    }
