...
class Growtype_Quiz_Result
{
    ...
    {
        if (get_option('growtype_quiz_results_page')) {
            return;
        }

        $slug = trim(self::RESULTS_PAGE_SLUG, '/'); // 'gqresults'
    ...
    {
        if (!get_query_var('growtype_quiz_results') || get_option('growtype_quiz_results_page')) {
            return;
        }
    ...
    {
        return Growtype_Quiz_Result_Crud::get_quiz_single_result_data_by_unique_hash($unique_hash) ?: null;
    }
    ...
    public static function get_quiz_data(array $result): array
    {
        ...
        if ($quiz_slug) {
            ...
            if ($entry && !empty($entry['__class']) && class_exists($entry['__class'])) {
                return (new $entry['__class']())->to_quiz_data();
            }
        }
