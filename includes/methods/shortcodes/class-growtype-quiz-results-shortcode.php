...
class Growtype_Quiz_Results_Shortcode
{
    function __construct()
    {
        if (!wp_is_json_request() && !is_admin()) {
            add_shortcode("growtype_quiz_results", [
                $this,
                "growtype_quiz_results_shortcode",
            ]);
        }
    }
    ...
    {
        $quiz_results = growtype_quiz_get_extended_user_quizes_results(
            $attr["user_id"] ?? null,
            $attr["quiz_id"] ?? null,
        );
        ...
        if (!empty($quiz_results)) {
            ...
            foreach ($quiz_results as $questions) {
                foreach ($questions as $question_key => $question) {
                    // '_meta' holds quiz metadata, not a renderable question — skip it.
                    if ($question_key === "_meta" || !is_array($question)) {
                        continue;
                    }

                    $question_intro =
                        isset($question["question_title"]) &&
                        !empty($question["question_title"])
        ...
        $results_content = apply_filters(
            "growtype_quiz_results_shortcode_content",
            $results_content,
            $quiz_results,
            $attr,
        );
